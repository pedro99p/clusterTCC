...
def run(self_ip):
    ...
    policy = DefaultHydroPolicy(scaler, scale_increase=0, min_utilization=0)
    ...
    while True:
        ...
        if (restart_pull_socket in socks and socks[restart_pull_socket] ==
            ...
            args = msg.split(':')
            logging.info('restart_pull_socket ip' + str(args[1]))
        ...
        if end - start > REPORT_PERIOD:
            ...
            logging.info('function_runtimes log: %s', function_runtimes)
            logging.info('function_frequencies log: %s', function_frequencies)
            logging.info('dag_runtimes log: %s', dag_runtimes)
            logging.info('arrival_times log: %s', arrival_times)
